...
class ProgressiveEBM(object):
	def __init__(self, args):
		...
		self.create_nets(self.max_step - self.min_step + 1, args)
		self.create_ema(self.ebm[str(self.max_step)], args)
	...
	def create_ema(self, model, args):
		...
		self.ema_manager = EMA(model, self.ebm_ema)
	...
	def switch_optimzer(self, step):
		self.optimizer = torch.optim.Adam(self.ebm[str(step)].parameters(), lr=self.args.lr_default, betas=(0.0, 0.99))
		...
		for index, s in enumerate(step_list):
			...
			for k in range(langevin_steps):
				...
				if self.args.ema and sampling:
					...
					loss = self.ebm[str(s)](x_k, step=s, alpha=alphas[s], label=label).sum() + beta * (-x_k**2 / (2*e**2)).sum()
				...
				sigma = self.polynomial(k, langevin_steps, base_lr=1.5e-2, end_lr=1e-3, power=1) if self.args.cyclic else 1e-2
...
if __name__ == "__main__":
	...
	if args.sched:
		args.lr = {8: 0.0005, 16: 0.0008, 32: 0.0010, 64: 0.0012, 128: 0.0015}
		args.batch = {8: 128, 16: 128, 32: 64, 64: 64, 128: 32, 256: 32}
		# args.phases = {8:args.phase, 16:args.phase, 32: args.phase // 2, 64: args.phase // 2, 128: args.phase // 4}
