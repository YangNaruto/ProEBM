...
class ProgressiveEBM(object):
	def __init__(self, args):
		...
		self.ebm = networks.Discriminator(base_channel=args.base_channel, spectral=args.spectral, res=args.res,
		                                  ...
		                                  from_rgb_activate=args.from_rgb_activate, bn=args.bn, split=args.split_bn).to(self.device)
		...
		self.optimizer = torch.optim.Adam(self.ebm.parameters(), lr=self.default_lr, betas=(0.0, 0.999), weight_decay=0.000, amsgrad=args.amsgrad)
	...
	def langevin_sampler(self, x_k, sampler_step, step, alpha, from_beginning=False, label=None, soft=False, sampling=False):
		...
		for index, s in enumerate(step_list):
			...
			for k in range(langevin_steps):
				...
				lr = self.cyclic(langevin_steps, k, M=1, lr=self.args.langevin_lr, min_lr=1e-2) if self.args.cyclic else self.args.langevin_lr
				sigma = self.cyclic(langevin_steps, k, M=1, lr=1e-1, min_lr=1e-3) if self.args.cyclic else 5e-3
...
if __name__ == "__main__":
	...
	parser.add_argument('--momentum', default=0.999, type=float, help='ema momentum')
	...
	if args.mode == "train":
		...
		if args.sched:
			args.lr = {8: 0.0004, 16: 0.0008, 32: 0.0010, 64: 0.0010, 128: 0.0010}
			args.batch = {8: 256, 16: 128, 32: 128, 64: 64, 128: 64, 256: 32}
